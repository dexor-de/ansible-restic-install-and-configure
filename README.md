...
|restic_exports: | includes all env variables to connect to s3 storageand init repository
...
```yaml
- name: install and configure restic on client
  hosts: i2temp
  vars:
    restic_user: backup
    restic_profiles:
